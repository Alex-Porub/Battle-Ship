...
function typingString (selector, speed=60, delay=300) {
    ...
    descriptionText_text.className = "descriptionText__text";
    let descriptionText_line = document.createElement('span');
    descriptionText_line.className = "descriptionText__line";
...
const boxOfShips = document.querySelector('.playbox__boxOfShips');
const battleArea =[];

const allShipsInTheBox = boxOfShips.querySelectorAll('.ship');
for (eachShip of allShipsInTheBox){
    eachShip.dataset.left = eachShip.offsetLeft;
    eachShip.dataset.top = eachShip.offsetTop; 
}
for (eachShip of allShipsInTheBox){  
    eachShip.style.left = eachShip.dataset.left+"px";
    eachShip.style.top = eachShip.dataset.top+"px";
    eachShip.style.position = "absolute";
}
...
function randomInteger(min, max) {    // случайное число от min до max
...
function allSellsFree(){
  ...
  // Расстановка кораблей на исходную позицию
for (eachShip of allShipsInTheBox){
    eachShip.dataset.orientation = "vertical";
    eachShip.dataset.translateX = 0;
    eachShip.dataset.translateY = 0;
}

function shipsPositioner_0(){
    allSellsFree();
    for (eachShip of allShipsInTheBox){
        eachShip.classList.add("readyShip");
        eachShip.classList.remove("activeShip");
    }
}

const curentBattleAreaS =[];
            ...
            for (let i=0; i<mast; i++){
                ...
                if (i==0){
                    let activeShip = boxOfShips.querySelector('.ship:nth-of-type('+(parseInt(key)+1)+')'),
                        translate_x = cellOccuped.getBoundingClientRect().left-1 - activeShip.getBoundingClientRect().left + parseInt(activeShip.dataset.translateX),
                        translate_y = cellOccuped.getBoundingClientRect().top-1 - activeShip.getBoundingClientRect().top + parseInt(activeShip.dataset.translateY),
                        rotateAngle = 0;                    
                    ...
                    activeShip.style.transform = "translate("+(320-parseInt(key)*40)+"px, -420px) rotate("+(rotateAngle)+"deg) scale(1.6, 1.6)";
                    activeShip.style.boxShadow = "18px 8px 30px 2px rgba(0,0,0,0.45)";
                    activeShip.style.opacity = "0.5";

                    setTimeout(() => {
                        activeShip.style.transform = "translate("+(510-parseInt(key)*40)+"px, -180px) rotate("+(rotateAngle+120)+"deg) scale(2.0, 2.0)";
                        activeShip.style.boxShadow = "14px 14px 36px 4px rgba(0,0,0,0.45)";
                        activeShip.style.opacity = "0.7";                                
                    }, 300);

                    setTimeout(() => {
                        activeShip.style.transform = "translate("+(320-parseInt(key)*40)+"px, 30px) rotate("+(rotateAngle+240)+"deg) scale(1.6, 1.6)";
                        // activeShip.style.transformOrigin = "";
                        activeShip.style.boxShadow = "14px 14px 36px 4px rgba(0,0,0,0.45)";
                        activeShip.style.opacity = "";
                    }, 600);

                    setTimeout(() => {
                        activeShip.style.transform = "translate("+translate_x+"px, "+translate_y+"px) rotate("+(rotateAngle+360)+"deg) scale(1.1, 1.04)"; 
                ...
                if (battleArea[rightTopRow]&&battleArea[rightTopRow][rightTopCol]&&battleArea[rightTopRow][rightTopCol]=="free"){
                    battleArea[rightTopRow][rightTopCol] = "bordering";
                }                
                if (battleArea[rightBotRow]&&battleArea[rightBotRow][rightBotCol]&&battleArea[rightBotRow][rightBotCol]=="free"){
                ...
                if (position.side=="down"){row++;}else col++;
            }   
        }, time);
    }
}
